...
class QuillEditorState extends State<QuillEditor> implements EditorTextSelectionGestureDetectorBuilderDelegate {
  ...
  void initState() {
    ...
    _selectionGestureDetectorBuilder = _QuillEditorSelectionGestureDetectorBuilder(this, config.detectWordBoundary);
  ...
  Widget build(BuildContext context) {
    ...
    if (kIsWeb) {
      ...
      return KeyboardListener(
        onKeyEvent: (_) {},
        focusNode: FocusNode(onKeyEvent: (node, event) => KeyEventResult.skipRemainingHandlers),
        child: editor,
      );
    }
...
class _QuillEditorSelectionGestureDetectorBuilder extends EditorTextSelectionGestureDetectorBuilder {
  ...
  void onSingleLongTapMoveUpdate(LongPressMoveUpdateDetails details) {
    if (_state.config.onSingleLongTapMoveUpdate != null) {
      if (renderEditor != null &&
          _state.config.onSingleLongTapMoveUpdate!(details, renderEditor!.getPositionForOffset)) {
        return;
      }
    }
    ...
    if (Theme.of(_state.context).isCupertino) {
      renderEditor!.selectPositionAt(from: details.globalPosition, cause: SelectionChangedCause.longPress);
    } else {
  ...
  void onTapDown(TapDownDetails details) {
    if (_state.config.onTapDown != null) {
      if (renderEditor != null && _state.config.onTapDown!(details, renderEditor!.getPositionForOffset)) {
        return;
      }
    }
  ...
  void onSingleLongTapStart(LongPressStartDetails details) {
    if (_state.config.onSingleLongTapStart != null) {
      if (renderEditor != null && _state.config.onSingleLongTapStart!(details, renderEditor!.getPositionForOffset)) {
        return;
      }
    }
  ...
  void onSingleLongTapEnd(LongPressEndDetails details) {
    if (_state.config.onSingleLongTapEnd != null) {
      if (renderEditor != null) {
        if (_state.config.onSingleLongTapEnd!(details, renderEditor!.getPositionForOffset)) {
          return;
        }
...
class RenderEditor extends RenderEditableContainerBox
  ...
  void handleDragStart(DragStartDetails details) {
    ...
    final newSelection = selectPositionAt(from: details.globalPosition, cause: SelectionChangedCause.drag);
  ...
  TextPosition getPositionForOffset(Offset offset) {
    ...
    return TextPosition(offset: localPosition.offset + child.container.offset, affinity: localPosition.affinity);
  }
  ...
  FloatingCursorPainter get _floatingCursorPainter =>
      FloatingCursorPainter(floatingCursorRect: _floatingCursorRect, style: _cursorController.style);
  ...
  TextRange getWordBoundary(TextPosition position) {
    ...
    final localPosition = TextPosition(offset: position.offset - nodeOffset, affinity: position.affinity);
    final localWord = child.getWordBoundary(localPosition);
    return TextRange(start: localWord.start + nodeOffset, end: localWord.end + nodeOffset);
  }
